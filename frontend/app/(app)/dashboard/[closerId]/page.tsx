...
type ClienteRow = {
  ...
  coluna: ColunaLite | null;
} & Record<ChecklistField, boolean>;

type CloserDetailResp = {
...
export default async function CloserDetalhePage({
  ...
  const { ok, data } = await apiServerJson<CloserDetailResp>(`/api/views/closer/${closerId}`);
  if (!ok || !data?.closer) redirect("/dashboard");
  const { closer, categorias, colunas, clientes } = data;
      ...
      <section className="relative overflow-hidden rounded-3xl border border-white/10 bg-brand-gradient p-6 text-white shadow-brand animate-rise sm:p-8">
        ...
        <div className="relative flex flex-wrap items-center justify-between gap-6">
          <div className="flex items-center gap-4 sm:gap-5">
            ...
            <img
              src={apiUrl(`/api/closers/${closer.id}/avatar`)}
              alt={closer.nome}

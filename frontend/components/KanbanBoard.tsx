...
export default function KanbanBoard({
  ...
  async function recarregar() {
    const [rc, rk] = await Promise.all([
      apiFetch(`/api/clientes${categoriaQS}`),
      apiFetch(`/api/colunas${categoriaQS}`),
    ]);
  ...
  async function novaColuna() {
    const res = await apiFetch("/api/colunas", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async function excluirColuna(id: string) {
    ...
    await apiFetch(`/api/colunas/${id}`, { method: "DELETE" });
  ...
  async function renomearCategoria() {
    ...
    await apiFetch(`/api/categorias/${edicao.id}`, {
      method: "PATCH",
      headers: { "Content-Type": "application/json" },
      body: JSON.stringify({ nome }),
    });
